...
class GameSprite(sprite.Sprite):
    def __init__(self, img, position, size, speed, animation=False):
        super().__init__()

        self.spritesheet = sprites[img]
        self.animation = animation
        self.animation_type = "idle"
        self.animation_frame = 0
        self.animation_timer = FPS * 0.25

        if self.animation:
            self.image = transform.smoothscale(
                image.load(self.spritesheet[self.animation_type][self.animation_frame]),
                size
            )
        else:
            self.image = transform.smoothscale(
                image.load(self.spritesheet[self.animation_type]),
                size
            )
    ...
    def reset(self):
        ...
        if self.animation:
            if self.animation_timer > 0:
                ...
                if self.animation_frame >= len(self.spritesheet[self.animation_type]):
                    self.animation_frame = 0
                self.animation_timer = FPS * 0.25

            self.image = transform.smoothscale(
                image.load(self.spritesheet[self.animation_type][0]),
